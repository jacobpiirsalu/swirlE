...
int robot_turn_ninety(int frequency_hz, double direction) {
    //double direction = -1.0;
    int counter = 0;
    double servo_pos = 0;
    //double direction = 1;	// switches between 1 &-1 in sweep mode
    ...
    while (running) {
        ...
        **/
        ...
        //ch = 7;
        if (rc_servo_send_pulse_normalized(7, servo_pos*.75) == -1) return -1;
        counter++;

        //ch = 8;
        if(rc_servo_send_pulse_normalized(8, servo_pos*.75)==-1) return -1;
...
int robot_move_forward_bullseye(int frequency_hz) {
    ...
    while (running) {
        ...
        **/
        ...
        //ch = 7;
        if (rc_servo_send_pulse_normalized(7, servo_pos*max_speed) == -1) return -1;
