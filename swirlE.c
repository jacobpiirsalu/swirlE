...
//    printf("starting main\n");
    ...
    while(1) {
        ...
        if(!state[1] && !state[2]) { //line following state X00X
            ...
            //printf("\nsaw red\n");
            l_r_avg = 2*rolling_avg(l_red_arr, &l_red_val, &l_r_sum);
            //rc_usleep(20);
            r_r_avg = 2*rolling_avg(r_red_arr, &r_red_val, &r_r_sum);
            //rc_usleep(20);
            double l_green_val = colour_sensor_green(CS_OUT1);
            rc_usleep(20);
            double r_green_val = colour_sensor_green(CS_OUT2);
            rc_usleep(20);
            l_g_avg = 2*rolling_avg(l_green_arr, &l_green_val, &l_g_sum);
            //rc_usleep(20);
            r_g_avg = 2*rolling_avg(r_green_arr, &r_green_val, &r_g_sum);
            //rc_usleep(20);
            printf("%f %f\n",l_r_avg,r_r_avg);
            ...
            rc_usleep(20);
            l_b_avg = 2*rolling_avg(l_blue_arr, &l_blue_val, &l_b_sum);
            rc_usleep(20);
            r_b_avg = 2*rolling_avg(r_blue_arr, &r_blue_val, &r_b_sum);
            rc_usleep(20);
            //printf("\nsaw color\n");
